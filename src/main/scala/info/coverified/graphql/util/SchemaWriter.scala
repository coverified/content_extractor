...
object SchemaWriter {
  ...
  def write(
    ...
    s"""${packageName.fold("")(p => s"package $p\n\n")}${if (hasTypes && hasOperations) s"import ${packageName.fold("")(p => s"$p")}.${objectName.fold("")(o => o)}.Types._\n" else ""}
          ${if (typesAndOperations.contains("@GQL")) "import caliban.schema.Annotations._" else ""}
