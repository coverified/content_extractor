...
import info.coverified.graphql.schema.CoVerifiedClientSchema.{Entry, Query, Url}
...
final case class Extractor(apiUrl: Uri) {
  ...
  def getAllUrls: ZIO[Console with SttpClient, Throwable, List[Url.UrlView]] = {
    // get existent urls
  ...
  def getAllConfigs(cfgFolderPath: File): Map[String, ProfileConfig] = {
    if (cfgFolderPath.exists() && cfgFolderPath.isDirectory) {
      cfgFolderPath.listFiles.filter(_.isFile).map(file => ProfileConfig(ConfigFactory.parseFile(file)))
        .map(profileCfg => profileCfg.profile.hostname -> profileCfg).toMap
    } else {
      Map.empty
    }
  }

  def getProfile4Url(url: String, configs: Map[String, ProfileConfig]): Option[ProfileConfig] = {
    configs.find {
      case (hostname, cfg) =>
  ...
  def getMutations(url: String, cfg: ProfileConfig) = {
    Analyzer.run(url, cfg).map(mut => Connector.sendRequest(mut.toRequest(apiUrl)))
  }
